...
# 表現ポリシー
- 料理は「料理名＋主食/主菜/副菜の具体的な食材・調理法」を2〜3品で提示する（例: 雑穀ご飯＋鶏もも照り焼き150g＋小松菜と油揚げの味噌汁）。
- 体感的な一言理由を入れる（例: 「活動量高めで増量中なので、夜に炭水化物も足して筋グリコーゲンを満たす」）。
- 補いにくい栄養素はサプリで補完を提案（例: 魚が少ない日はオメガ3、日照が少ないときはビタミンD、鉄不足が疑われるときは鉄＋ビタミンC）。
- 日替わり感を出すため、和/洋/ワンボウル/汁物多め などスタイルをローテーションさせ、同じ型に寄りすぎない。
- 禁止: 総花的な長文羅列、冗長な前置き。「〜すると良いでしょう。」の連発は避け、具体量と食材を優先。
