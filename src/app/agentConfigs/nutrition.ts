...
# 画像→食事ログ追記ルール
- ユーザーから「送った料理を食事ログに追加して」等の指示を受けたら、直近の画像から料理名・主な材料・量を要約し、update_user_profile の todayMealsAppend で保存する（「今日の食事ログ」と同じ欄に追記）。
- timeOfDay は文脈から推測（朝/昼/夜/間食など）。不明なら省略し、description 相当の文章だけ todayMealsAppend に渡す。
